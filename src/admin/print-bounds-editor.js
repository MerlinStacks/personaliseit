...
export class PrintBoundsEditor {
	/**
	...
	get scale() {
		return this.img.naturalWidth > 0
			? this.img.offsetWidth / this.img.naturalWidth
			: 1;
	}
	...
	destroy() {
		...
		this.sel
			.querySelector( '.oc-handle-se' )
			?.removeEventListener( 'mousedown', this._onResizeDown );
	}
	...
	_bindEvents() {
		...
		this.sel
			.querySelector( '.oc-handle-se' )
			?.addEventListener( 'mousedown', this._onResizeDown );
	}

	_handleDraw( e ) {
		if (
			e.target === this.sel ||
			e.target.classList.contains( 'oc-handle-se' )
		) {
			return;
		}
		e.preventDefault();
		...
		document.addEventListener(
			...
			( me ) => {
				this.sel.style.left =
					Math.max(
						0,
						Math.min( startL + me.clientX - startX, maxL )
					) + 'px';
				this.sel.style.top =
					Math.max(
						0,
						Math.min( startT + me.clientY - startY, maxT )
					) + 'px';
				this.onChange( this.getBounds() );
			},
			{ signal }
	...
	_handleResize( e ) {
		...
		document.addEventListener(
			'mousemove',
			( me ) => {
				this.sel.style.width =
					Math.max( 10, startW + me.clientX - startX ) + 'px';
				this.sel.style.height =
					Math.max( 10, startH + me.clientY - startY ) + 'px';
				this.onChange( this.getBounds() );
			},
			{ signal }
		);

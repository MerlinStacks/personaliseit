...
function initColourModal() {
	// Add colour button.
	document.getElementById( 'oc-add-colour-btn' )
		?.addEventListener( 'click', () => openColourModal( null ) );

	// Close.
	document.getElementById( 'oc-colour-modal-close' )
		?.addEventListener( 'click', closeColourModal );
	document.getElementById( 'oc-colour-cancel-btn' )
		?.addEventListener( 'click', closeColourModal );
	...
	// Save.
	document.getElementById( 'oc-colour-save-btn' )
...
function buildGroupCardEl( group ) {
	...
	const more = group.colourIds.length > 10
		? `<span class="oc-group-card-more">+${ group.colourIds.length - 10 }</span>` : '';

	const empty = group.colourIds.length === 0
...
function selectedColourIds() {
	return [ ...( groupPicker()?.querySelectorAll( 'input:checked' ) || [] ) ]
...
function initGroupModal() {
	document.getElementById( 'oc-create-colour-group-btn' )
		?.addEventListener( 'click', () => openGroupModal( null ) );

	document.getElementById( 'oc-colour-group-modal-close' )
		?.addEventListener( 'click', closeGroupModal );
	document.getElementById( 'oc-colour-group-cancel-btn' )
		?.addEventListener( 'click', closeGroupModal );
	...
	document.getElementById( 'oc-colour-group-save-btn' )

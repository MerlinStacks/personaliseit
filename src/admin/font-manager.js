...
( function () {
	...
	function parseFontNames( buffer ) {
		...
		for ( let i = 0; i < count; i++ ) {
			...
			const isWin =
				platformID === 3 && encodingID === 1 && langID === 0x0409;
	...
	function subfamilyToProps( subfamily ) {
		...
		const style =
			s.includes( 'italic' ) || s.includes( 'oblique' )
				? 'italic'
				: 'normal';
		return { weight, style };
	}
	...
	async function handleFileSelected( file ) {
		...
		const isCurrentSelection = () =>
			fileSelectionToken === token && currentFile === token.file;
		...
		try {
			...
			currentFile = null;
			uploadError.textContent =
				'Could not read that font file. Please try another.';
		...
		if ( names && ! nameLocked ) {
			const family =
				names[ 16 ] || names[ 1 ] || filenameToTitle( file.name );
		...
		try {
			...
			previewText.style.fontFamily = '';
			uploadError.textContent =
				'Preview unavailable for this font. You can still upload it.';
			uploadError.style.display = '';
		}
	...
	function buildCardEl( font ) {
		...
		el.className =
			'oc-font-card' + ( font.active ? '' : ' oc-font-card--inactive' );
		...
		el.innerHTML = `
			<div class="oc-font-preview">
				<span class="oc-font-preview-text" style="font-family:'${ family }';font-weight:${
					font.weight
				};font-style:${ font.style };">AaBbCc 123</span>
			</div>
			<div class="oc-font-card-body">
				<div class="oc-font-card-title-row">
					<p class="oc-font-card-name" title="${ h( font.name ) }">${ h( font.name ) }</p>
					<span class="oc-badge ${
						font.active ? 'oc-badge-active' : 'oc-badge-inactive'
					}">${ font.active ? 'Active' : 'Inactive' }</span>
				</div>
				<div class="oc-font-card-meta">
					...
					${
						font.style === 'italic'
							? '<span class="oc-badge oc-badge-inactive">Italic</span>'
							: ''
					}
					${
						font.embroidery_suitable
							? '<span class="oc-badge oc-badge-active">Embroidery</span>'
							: ''
					}
				</div>
				<div class="oc-font-card-actions">
					${
						font.canPrintConvert
							? `<button type="button" class="oc-btn oc-btn-secondary oc-btn-sm oc-font-convert-btn" data-font-id="${ font.id }">Convert for print</button>`
							: ''
					}
					<a href="${ font.toggleUrl }" class="oc-btn oc-btn-secondary oc-btn-sm">${
						font.active ? 'Deactivate' : 'Activate'
					}</a>
					<a href="${
						font.deleteUrl
					}" onclick="return confirm('Delete this font?');" class="oc-btn oc-btn-danger oc-btn-sm">Delete</a>
				</div>
			</div>
		`;
		return el;
	...
	function openDetailPanel( fontId, familyName ) {
		...
		document
			.querySelectorAll( '.oc-font-card--active-detail' )
			.forEach( ( c ) =>
				c.classList.remove( 'oc-font-card--active-detail' )
			);
		const activeCard = document.querySelector(
			`.oc-font-card[data-font-id="${ fontId }"]`
		);
		if ( activeCard ) {
	...
	function renderDetailVariants( familyName ) {
		...
		if ( ! variants.length ) {
			detailVariants.innerHTML =
				'<p style="padding:16px 20px;color:var(--oc-gray-400);font-size:13px;">No variants found.</p>';
			return;
		}

		detailVariants.innerHTML = variants
			.map( ( f ) => {
				injectFontFace( f );
				const family = `oc-preview-${ f.id }`;
				return `
				<div class="oc-detail-variant" data-font-id="${ f.id }">
					<div class="oc-detail-variant-preview" style="font-family:'${ family }';font-weight:${
						f.weight
					};font-style:${ f.style };">AaBbCc 123</div>
					<div class="oc-detail-variant-meta">
						...
						${
							f.embroidery_suitable
								? '<span class="oc-badge oc-badge-active">Embroidery</span>'
								: ''
						}
						<span class="oc-badge ${
							f.active ? 'oc-badge-active' : 'oc-badge-inactive'
						}">${ f.active ? 'Active' : 'Inactive' }</span>
					</div>
					<div class="oc-detail-variant-actions">
						${
							f.canPrintConvert
								? `<button type="button" class="oc-btn oc-btn-secondary oc-btn-sm oc-font-convert-btn" data-font-id="${ f.id }">Convert for print</button>`
								: ''
						}
						<a href="${ f.toggleUrl }" class="oc-btn oc-btn-secondary oc-btn-sm">${
	...
	async function convertFontInBrowser( font ) {
		...
		if ( ! response.ok ) {
			throw new Error(
				`Could not load font file (${ response.status }).`
			);
		}
		...
		fd.append(
			'oc_font_file',
			blob,
			`${ safeFilename( font.name ) || 'font' }-print.ttf`
		);
		...
		if ( ! json.success ) {
			throw new Error(
				json.data?.message || 'Browser conversion failed.'
			);
		}
	...
	function applyConvertedFont( updated ) {
		...
		window.alert(
			'Font converted for print. Existing designs will keep using this font.'
		);
	}
	...
	async function saveRename() {
		...
		try {
			...
			document
				.querySelectorAll(
					`.oc-font-card[data-font-name="${ CSS.escape( oldName ) }"]`
				)
				.forEach( ( card ) => {
	...
	function renderFontPicker( selectedIds ) {
		...
		groupFontPicker.innerHTML = families
			.map( ( f ) => {
				...
				return `
				<label class="oc-group-font-item${
					checked ? ' oc-selected' : ''
				}" data-font-id="${ f.id }">
					<input type="checkbox" value="${ f.id }" ${ checked } />
					<span class="oc-group-font-preview-mini" style="font-family:'oc-preview-${
						f.id
					}';font-weight:${ f.weight };font-style:${
						f.style
					};">Aa</span>
		...
		try {
			...
			if ( editingGroup ) {
				...
				const card = groupGrid.querySelector(
					`.oc-group-card[data-group-id="${ saved.id }"]`
				);
				if ( card ) {
	...
	function refreshGroupCard( el, group ) {
		...
		const preview = members
			...
			.map( ( f ) => {
				injectFontFace( f );
				return `<span style="font-family:'oc-preview-${
					f.id
				}';font-weight:${ f.weight };font-style:${
					f.style
				};" title="${ h( f.name ) }">Aa</span>`;
			} )
			.join( '' );
		const more =
			group.fontIds.length > 5

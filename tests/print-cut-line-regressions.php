...
// phpcs:ignore Universal.NamingConventions.NoReservedKeywordParameterNames.defaultFound -- Preserve the WordPress stub parameter contract.
...
foreach ( [ -20.1, -10.1, 120.1, 70.1 ] as $i => $expected ) {
	$check( $near( $bounds[ $i ], $expected ), 'Overflow bounds' );
}
$ordinary = [
	'layers' => [
		[
...
foreach ( $dom->getElementsByTagName( '*' ) as $node ) {
	if ( ! $node->hasAttribute( 'stroke' ) ) {
		continue; // New normalization groups inherit paint.
	}
...
foreach ( [ '', '<svg>', '<svg xmlns="http://www.w3.org/2000/svg"><image href="bad.png"/></svg>', '<svg xmlns="http://www.w3.org/2000/svg"><path d="M0 0L1 1"/></svg>' ] as $bad ) {
	...
	try {
		$invoke( 'cut_line_svg', $invalid, 140.0, 80.0 );
		$rejected = false;
...
foreach ( $matches[1] as $stream ) {
	// phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged -- PDF streams may be uncompressed; failed decompression intentionally falls back to the original bytes.
	$decoded  = @gzuncompress( $stream );
	$streams .= $decoded ? $decoded : $stream;
}
...
foreach ( $matches[1] as $stream ) {
	// phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged -- PDF streams may be uncompressed; failed decompression intentionally falls back to the original bytes.
	$decoded           = @gzuncompress( $stream );
	$combined_streams .= $decoded ? $decoded : $stream;
}
...
$probe           = new class() extends TCPDF {
	...
	public function ImageSVG( $_file, $_x = null, $_y = null, $_w = 0, $_h = 0, $_link = '', $_align = '', $_palign = '', $_border = 0, $_fitonpage = false ) {
		$this->events[] = 'svg';
	}
...
foreach ( [ 'colour', 'spot' ] as $print_mode ) {
	$probe = new class() extends TCPDF {
	...
	$invoke( 'render_layer_payload', $probe, $area, $data, 0.0, 0.0, $print_mode );

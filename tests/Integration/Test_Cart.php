...
class Test_Cart extends WC_Unit_Test_Case {
	...
	public function optional_layer_costs_preserve_base_fee_and_scale_with_cart_quantity(): void {
		...
		foreach ( [
			...
			$this->assertSame( $expected, array_sum( array_map( static fn( $fee ) => (float) $fee->amount, WC()->cart->get_fees() ) ) );
		}
		$item = new WC_Order_Item_Product();
		$integration->save_to_order_item( $item, $key, $values, new WC_Order() );
		$this->assertSame( $values['_oc_layer_costs'], $item->get_meta( '_oc_layer_costs' ) );
		$this->assertSame( '24.00', $item->get_meta( '_oc_personalisation_fee_total' ) );
	}

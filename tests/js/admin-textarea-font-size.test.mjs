...
function previewSize( settings, scale = 1, type = 'textarea' ) {
	...
	const create = vm.runInNewContext( `${ code }; createLayerPreviewRenderer`, {
		window,
		...
		multilineTextboxFits: ( object, width, height ) => object.height <= height,
	} );
...
test( 'textarea preview honours explicit defaults instead of shrinking to the minimum', () => {
	assert.equal( previewSize( { default_font_size: 24, min_font_size: 8 } ), 24 );
	assert.equal( previewSize( { default_font_size: 32, min_font_size: 8 } ), 32 );
	assert.equal( previewSize( { default_font_size: 24, min_font_size: 12 } ), 24 );
} );

test( 'explicit textarea defaults still respect bounds and preview scale', () => {
	assert.equal( previewSize( { default_font_size: 24, max_font_size: 20 } ), 20 );
	assert.equal( previewSize( { default_font_size: 6, min_font_size: 8 } ), 8 );
...
test( 'automatic textarea sizing and single-line fitting remain enabled', () => {
	...
	assert.equal( previewSize( { default_font_size: 0, min_font_size: 12 } ), 12 );

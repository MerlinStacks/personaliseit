...
function sanitize_hex_color( $value ) {
	return preg_match( '/^#[a-f0-9]{6}$/i', $value ) ? $value : null;
}
function esc_url_raw( $value ) {
	return $value;
}
function wp_json_encode( $value ) {
	// phpcs:ignore WordPress.WP.AlternativeFunctions.json_encode_json_encode -- Implement the standalone wp_json_encode stub without recursion.
...
class OC_DB {
	...
	public static function assignment_allows_design( $assignment, $id ) {
		return 1 === $id;
	}
	...
	public static function get_design_print_areas( $id ) {
		return [
			(object) [
				'id'      => 1,
				'visible' => 1,
	...
	public static function get_design_layers( $id ) {
		return self::$layers;
	}
	public static function get_fonts( $active ) {
		return [];
	}
	public static function get_colours( $active ) {
		return [];
	}
	public static function get_image_filters( $active ) {
		return [
			(object) [
				'id'         => 7,
				'filter_key' => 'ai',
...
class OC_Upload_Handler {
	public static function admin_default_attachment_is_valid( $id ) {
		return 10 === $id;
	}
	public static function attachment_is_accepted( $id, ...$args ) {
		return in_array( $id, [ 20, 21, 22 ], true );
	}
	public static function attachment_primary_context( $id ) {
		return null;
	}
}
...
function layer( $id = 1, $layer_type = 'text', $settings = [], $properties = [] ) {
...
function normalise( $input ) {
	return OC_Cart::normalise_v2_layers( 100, 0, 1, $input );
}
...
foreach ( [ 'ai_image', 'spotify', 'lineart', 'clipart', 'night_sky', 'mask', 'cut_line' ] as $layer_type ) {
	check( ! OC_Cart::normalise_layer_settings( [ 'additional_cost_enabled' => true ], $layer_type )['additional_cost_enabled'], 'Unsupported type cannot charge' );
}
		...
		2.25 === amount(
			[
				1 => [
					'value'           => 'Alice',
					'additional_cost' => -999,
					'settings'        => [ 'additional_cost_enabled' => false ],
				],
			]
		),
		'Only saved amounts are trusted'
	);
}
...
foreach ( [ 'image', 'clipmask' ] as $layer_type ) {
	OC_DB::$layers = [ layer( 1, $layer_type, [ 'default_attachment_id' => 10 ] ) ];
	foreach (
		[
			[],
			[ 'attachmentId' => 0 ],
			...
			is_wp_error(
				normalise(
					[
						1 => [
							'attachmentId'       => 20,
							'sourceAttachmentId' => $forged_source,
						],
					]
				)
			),
			'Cannot forge default/empty/unrelated source'
		);
	}
	check( is_wp_error( normalise( [ 1 => [ 'attachmentId' => 999 ] ] ) ), 'Unvalidated upload rejected' );
	$GLOBALS['meta'][21] = [
		'_oc_ai_filter_id'        => 7,
		'_oc_ai_filter_source_id' => 10,
	];
	$GLOBALS['meta'][22] = [
		'_oc_ai_filter_id'        => 7,
		'_oc_ai_filter_source_id' => 20,
	];
	check(
		0.0 === amount(
			[
		...
		'Verified default derivative is free'
	);
	...
	check(
		...
		'Verified custom derivative charges'
	);
	check(
		is_wp_error(
			normalise(
		...
		'Custom derivative cannot claim default source'
	);
}
OC_DB::$layers = [
	layer(
...
check(
	...
	'Selected verified AI effect on default is free'
);
check(
	2.25 === amount(
		[
	...
	'Selected verified AI effect on custom photo charges'
);
OC_DB::$layers = [
	layer(
		1,
...
$data        = $integration->add_cart_item_data(
	[
		'_oc_submission_raw' => $raw,
		'_oc_flat_rate'      => -100,
		'_oc_layer_costs'    => [ 'forged' ],
	],
	100,
	0
);

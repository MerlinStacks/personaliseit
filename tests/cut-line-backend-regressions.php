...
function wp_send_json_error( array $data, int $status = 200 ): never { throw new Json_Response( $data, $status ); }
...
class OC_DB {
	...
	public static function get_design( int $id ): object { return (object) [ 'id' => $id, 'active' => 1 ]; }
	public static function get_design_print_areas( int $id ): array { return self::$areas; }
	public static function get_design_layers( int $id ): array { return self::$layers; }
	public static function get_assignment_for_product( int $id, int $variant ): object { return (object) [ 'design_id' => 1 ]; }
	public static function assignment_allows_design( object $assignment, int $id ): bool { return 1 === $id; }
}
...
check( $clean === OC_Cut_Line::sanitize( $clean ), 'Sanitization is idempotent' );
...
foreach ( [ '96', '96px', '25.4mm', '2.54cm', '1in', '72pt', '6pc', '+9.6e1px', ' .96e2 ' ] as $length ) {
	...
	check( $normalized === OC_Cut_Line::sanitize( $normalized ), 'Normalized viewport is idempotent' );
}
...
foreach ( $invalid_viewports as $attributes ) {
	$error = OC_Cut_Line::sanitize( $viewport_svg( $attributes ) );
	check( is_wp_error( $error ) && 'invalid_cut_line' === $error->code, 'Reject invalid viewport early: ' . $attributes );
	check( str_contains( $error->get_error_message(), 'viewBox' ) && str_contains( $error->get_error_message(), 'width and height' ), 'Viewport error explains how to fix artwork' );
}
$save = new ReflectionMethod( OC_Admin_Products::class, 'normalise_design_layer_settings' );
check( [ 'cutLineSvg' => $safe ] === $save->invoke( null, [ 'cutLineSvg' => $unsafe, 'default_attachment_id' => 99 ], 'cut_line', 'engraving' ), 'Save re-sanitizes and drops attachment settings' );
try {
...
$area = (object) [ 'id' => 10, 'area_key' => 'front', 'label' => 'Front', 'print_method' => 'engraving', 'canvas_x' => 0, 'canvas_y' => 0, 'canvas_w' => 100, 'canvas_h' => 100, 'visible' => 1 ];
$cut = (object) [ 'id' => 20, 'area_id' => 10, 'type' => 'cut_line', 'label' => 'Private die', 'x' => -25, 'y' => -10, 'w' => 900, 'h' => 17, 'visible' => 1, 'locked' => 0, 'settings' => json_encode( [ 'cutLineSvg' => $svg ] ) ];
OC_DB::$areas = [ $area ];
OC_DB::$layers = [ $cut ];
$spec = OC_Render_Spec::build( 1, [ 20 => [ 'cutLineSvg' => $unsafe, 'attachmentId' => 99, 'x' => 0 ] ] );
...
foreach ( [ false => 403, true => 400 ] as $permission => $status ) {
	...
	try {
		...
		check( $status === $response->status, 'Upload requires permission and valid upload' );
	}
}

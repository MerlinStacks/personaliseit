...
(function () {
  ...
  function renderAreasList() {
    ...
    areas.forEach((area, i) => {
      ...
      item.querySelector('.oc-layer-delete-btn').addEventListener('click', e => {
        ...
        areas.splice(i, 1);
        if (selectedIndex === i) selectedIndex = areas.length > 0 ? Math.min(i, areas.length - 1) : -1;else if (selectedIndex > i) selectedIndex--;
        selectedLayerIndex = -1;
        snapshot();
        renderAll();
      });
  ...
  function renderLayerList(area) {
    ...
    layers.forEach((layer, li) => {
      ...
      item.querySelector('.oc-layer-delete-btn').addEventListener('click', e => {
        ...
        if (selectedLayerIndex === li) selectedLayerIndex = -1;else if (selectedLayerIndex > li) selectedLayerIndex--;
        snapshot();
        renderAll();
      });
      ...
      item.addEventListener('drop', e => {
        ...
        if (selectedLayerIndex === layerDragSrc) selectedLayerIndex = targetIdx;else if (layerDragSrc < targetIdx && selectedLayerIndex > layerDragSrc && selectedLayerIndex <= targetIdx) selectedLayerIndex--;else if (layerDragSrc > targetIdx && selectedLayerIndex >= targetIdx && selectedLayerIndex < layerDragSrc) selectedLayerIndex++;
        snapshot();
        renderAll();
      });
  ...
  function initInteractions() {
    ...
    document.querySelectorAll('.oc-layer-type-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        if (selectedIndex >= 0 && btn.dataset.type) addLayer(btn.dataset.type);
      });
    });
